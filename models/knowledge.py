...
from models.enumeration import Status, Entity, Action, Goal
from models.motion import neighbors, spins, known_path, path_to_spins



def perceive(knowledge, loc):
  ...
  if knowledge[loc] is not None and (knowledge[loc].pit == Status.Present or knowledge[loc].wumpus == Status.Present):
    # the agent has been killed and there are no perceptions
    return None
  # build perceptions
  wumpus, pit, gold = (Status.Absent, Status.Absent, Status.Absent,)
  # look neighboring cells to update perceptions
  for room in [knowledge[l] for l in neighbors(loc)]:
  ...
  if knowledge[loc].gold == Status.Present:
    gold = Status.Present  
